...
'''
def efReadCSV() :
    lines = csv.reader(open(r"kirc/feature.csv", "rb"))
    DataMat = list(lines)
    length = len(DataMat[0])
    feature = DataMat[0][2:(length-1)]
    sample = list(range(len(DataMat)-1))
    Class = list(range(len(DataMat)-1))
    tMatrix = list(range(len(DataMat)-1))
    for i in range(1, len(DataMat)):
        sample[i-1] = DataMat[i][0]
        Class[i-1] =DataMat[i][1]
        tMatrix[i-1] = [float(x) for x in DataMat[i][2:(length-1)]]
...
'''
def stageWise(xArr, yArr, eps = 1, delta = 0.99, numIt = 100):
...
Sample, Class, Feature, Matrix = fLoadDataMatrix(Args["Input"])
PNdict = {}
...
lsvc = svm.LinearSVC(C=0.15, penalty="l1", dual = False, random_state=5).fit(Matrix, ClassLabels)
lso = Lasso(alpha=0.01).fit(Matrix, ClassLabels)
...
#print(len(feature_idx_svm), len(feature_idx_lasso))
...
otherFeatureidx = [idxRankSort[:1], idxRankSort[:10], feature_idx_svm, feature_idx_lasso]
Re = []
for i in range(4):
    Re.append(CrossValidation(Matrix[:,otherFeatureidx[i]], ClassLabels, 10))
title2 = ['ttest-Top1', 'ttest-Top10', 'SVM', 'Lasso']
Histograms(title2, Re)
plt.savefig(Args["OutputDir"]+"/"+Args["H-Output2"])

#Cluster_Map
ClusterMap(Matrix[:,idxRankSort[:5]], ColList, [Feature[x] for x in idxRankSort[:5]], 't-test chosen 5 features')
